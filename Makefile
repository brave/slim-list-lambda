...
install-lambda:
	docker run --rm -v $(PWD):/var/task public.ecr.aws/sam/build-nodejs24.x ./build.sh
...
bundle:
	...
	find $(TMP_WORKSPACE)/node_modules/adblock-rs/js -mindepth 1 -not -name index.js -not -name index.node -exec rm -rf {} +
